...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
  ...
  if (!text) throw `1} _${usedPrefix + command} https://vm.tiktok.com/ZM686Q4ER/_`;
  if (!/(?:https:?\/{2})?(?:w{3}|vm|vt|t)?\.?tiktok.com\/([^\s&]+)/gi.test(text)) throw `2} _${usedPrefix + command} https://vm.tiktok.com/ZM686Q4ER/_`;
  const texto = `يتم التحميل يصحبي🤖⌚`;
  ...
  try {
    ...
    const desc1n = `4[0]} _${usedPrefix}tomp3_ 4[1]}`;
  ...
  try {
    ...
    const desc1 = `5[0]} _${usedPrefix}tomp3_ 5[1]}`;
    await conn.sendMessage(m.chat, {video: {url: dataF.play}, caption: desc1}, {quoted: m});
  } catch (e1) {
    try {
      const tTiktok = await tiktokdlF(args[0]);
      // let desc2 = `🔗 *Url:* ${tTiktok.video}`
      const desc2 = `6[0]} _${usedPrefix}tomp3_ 6[1]}`;
      await conn.sendMessage(m.chat, {video: {url: tTiktok.video}, caption: desc2}, {quoted: m});
    } catch (e2) {
      try {
        const p = await fg.tiktok(args[0]);
        // let te = `*𝚄𝚂𝙴𝚁𝙽𝙰𝙼𝙴:* ${p.author || 'Indefinido'}`
        const te = `_${usedPrefix}tomp3_`;
...
handler.command = /^(توك|ttdl|tiktokdl|tiktoknowm|tt|ttnowm|tiktokaudio)$/i;
export default handler;

async function tiktokdlF(url) {
  if (!/tiktok/.test(url)) return `10} _${usedPrefix + command} https://vm.tiktok.com/ZM686Q4ER/_`;
